...
try {{
    ...
    xhr.onload = function () {{
        if (xhr.getResponseHeader("Content-Type") === "application/json") {{
            ...
            callback({{"ok": false, "status": xhr.status, "statusText": xhr.statusText, "error": "Non-JSON response received"}});
        }}
    }};
...
"""
            ...
            if response and response["ok"]:
               try:                    
                   ...
                   self.logger.error(f"Could not parse response: {response['response']}")
                   raise Exception(f"Could not parse response: {response['response']}")

            else:
                print("JavaScript error: ", response.get("error"))
                return None
